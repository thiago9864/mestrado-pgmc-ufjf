...
//     double t;
   ...
   public:
    ...
    virtual void vector_value(const Point<dim> &p, Vector<double> &values) const {
        ...
        // // ux
        // values[0] = -cos(p[0]) * sin(p[1]) * exp((-2.0 * t) / re);

        // // uy
        // values[1] = sin(p[0]) * cos(p[1]) * exp((-2.0 * t) / re);

        // // p
        // values[2] = -0.25 * (cos(p[0]) + cos(p[1])) * exp((-4.0 * t) / re);
   ...
   public:
    NavierStokesCG(const unsigned int degree)
        ...
        : degree(degree),
          ...
          // time_step(time_end / time_max_number),  // dt
          time(0),               // t^n
        ...
        for (const auto &cell : dof_handler.active_cell_iterators()) {
            ...
            for (unsigned int q = 0; q < n_q_points; ++q) {
                ...
                for (unsigned int i = 0; i < dofs_per_cell; ++i) {
                    ...
                    const double div_phi_i_v = fe_values[velocities].divergence(i, q);         // div v
                    ...
                    for (unsigned int j = 0; j < dofs_per_cell; ++j) {
                        ...
                        const double div_phi_j_u = fe_values[velocities].divergence(j, q);         // div u
                        ...
                        local_matrix(i, j) += (((visc * time_step) / rho) * scalar_product(grad_phi_i_v, grad_phi_j_u)  // (mu * dt)/rho * (grad_v, grad_u)
                                               + phi_j_u * phi_i_v                                                      // (u, v)
                                               + time_step * grad_phi_j_u * old_solution_values[q] * phi_i_v            //  dt * grad_u * u_old * v // Termo convectivo
                                               - (time_step / rho) * phi_i_q * div_phi_j_u                              // -(dt/rho) * (q,div_u)
                                               - (time_step / rho) * phi_j_p * div_phi_i_v                              // -(dt/rho) * (p,div_v)
                                               - 1e-8 * (time_step / rho) * phi_i_q * phi_j_p)                          // -(dt/rho) * (p,q)   -> Isso elimina os valores zero na diagonal principal
                                              * fe_values.JxW(q);                                                       // Esse J e W já incluem a função peso e W da integração
                    }

                    local_rhs(i) += (prev_solution_values[q] * phi_i_v) * fe_values.JxW(q);  // (u_prev, v)
                }
            }
