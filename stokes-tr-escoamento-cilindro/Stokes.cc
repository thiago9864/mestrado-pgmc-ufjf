...
double u_max;
double time_end;
double omega_altura = 0.41;
double omega_largura = 2.2;
double diam_circulo = 0.1;
...
class BoundaryValues : public Function<dim> {
   ...
   public:
    ...
    virtual void vector_value(const Point<dim> &p, Vector<double> &values) const {
        ...
        // velocidade na entrada e saida
        ux = (4.0 * u_max * y * (omega_altura - y)) / std::pow(omega_altura, 2);

        if (abs(x) < d) {
            // entrada, x=0
            values[0] = ux;
        } else if (abs(x - omega_largura) < d) {
            // saida, x=2.2
            values[0] = ux;
        }
    }
...
class NavierStokesCG {
   public:
    ...
    void run() {
        ...
        coef_drag_map.clear();
        coef_lift_map.clear();
        dif_pressao_map.clear();

        printf("\n***Inicia a execucao***\n");

        // Inicializa malha
        // Aqui define o mesh do cilindro
        ...
        std::ofstream mesh_svg_file("mesh_t0.svg");
        ...
        time = 0.0;
        timestep_number = 0;
        reynolds = (2.0 * u_max * diam_circulo) / (3.0 * visc);
        condicao_inicial();
        ...
        for (timestep_number = 1; condicao_de_parada(is_aprox_estacionario); timestep_number++) {
            ...
            // Inicia solução anterior do método de Picard (u^{n+1/2 (bar)} = u^{n})
            ...
            do {
                ...
                // Calcula solução não linear u^{n+1/2}
                assemble_system();
                solve();
            ...
            // solution.block(0) += solution.block(0);       // 2 * u^{n+1/2}
            // solution.block(0) -= prev_solution.block(0);  // (2 * u^{n+1/2}) - u^{n}
            // constraints.distribute(aux);
            
            // BlockVector<double> aux = solution;
            // aux += solution;       // 2 * u^{n+1/2}
            // aux -= prev_solution;  // (2 * u^{n+1/2}) - u^{n}
            // constraints.distribute(aux);
            // solution = aux;

            solution.add(1., solution, -1., prev_solution);
            constraints.distribute(solution);
            
            printf("\nProblema nao linear com %d células, resolvido no passo de tempo %d (%.4f), com %d iteracoes\n",
            ...
            if (passo_output >= multPassos - 1) {
                ...
                // Calcula lift, drag e diferença de pressão
    ...
    void lerConfiguracoes(std::string arquivo_config) {
        ...
        while (std::getline(fin, line)) {
            ...
            if (line.find("visc") != std::string::npos) {
                ...
                sin >> rho;
            } else if (line.find("u_max") != std::string::npos) {
                sin >> u_max;
    ...
    void gera_arquivo_saida(double diff) {
        ...
        std::cout << "Tempo de execucao: " << diff << " s" << std::endl;
        ...
        resultado << "Tempo de execucao: " << diff << " s" << std::endl;
   ...
   private:
    ...
    std::map<double, double> coef_drag_map;
    std::map<double, double> coef_lift_map;
    std::map<double, double> dif_pressao_map;

    // Bloqueio de celulas oscilando
    ...
    void assemble_system() {
        ...
        const double rho_dt = (2.0 * rho) / time_step;
        ...
        for (const auto &cell : dof_handler.active_cell_iterators()) {
            ...
            double tau_u = 2.0 * pow(pow(2.0 / time_step, 2) + pow((2.0 * u_norm) / h, 2) + ((4.0 * visc) / (3.0 * h)), -0.5);
                ...
                for (unsigned int i = 0; i < dofs_per_cell; ++i) {
                    ...
                    for (unsigned int j = 0; j < dofs_per_cell; ++j) {
                        ...
                        local_matrix(i, j) += (visc * scalar_product(grad_phi_j_u, grad_phi_i_v)        // A:  mu * (grad_u, grad_v)
                                               + rho_dt * phi_j_u * phi_i_v                             // M:  (2 rho/dt) * (u, v)
                                               ...
                                               + rho_dt * phi_j_u * px_supg                             // S_M: (2 rho/dt) * u * p_x
                    ...
                    local_rhs(i) += (rho_dt * prev_solution_values[q] * phi_i_v    // F:   (2 rho/dt) * (u_prev, v)
                                     + rho_dt * prev_solution_values[q] * px_supg  // S_f: (2 rho/dt) * (u_prev, p_x)
                                     ) *
                                    fe_values.JxW(q);
                }
            }
    ...
    bool calcula_refinamento_fd(Vector<float> estimated_error_per_cell) {
        ...
        bool celula_travada;
        bool pode_travar = false;  // min_delta_entre_timesteps < 1e-4;
    ...
    /**
     * Calcula valores de lift, drag e a diferença de pressão na frente e atrás do cilindro
     * Autor: Lukas Bystricky (em Using Deal.II to Solve Problems in Computational Fluid Dynamics)
     */
    void calculate_lift_and_drag() {
        QGauss<dim - 1>
            face_quadrature_formula(3);
        const int
            n_q_points = face_quadrature_formula.size();
        const FEValuesExtractors::Vector velocities(0);
        const FEValuesExtractors::Scalar pressure(dim);
        std::vector<double>
            pressure_values(n_q_points);
        ...
        for (; cell != endc; ++cell) {
            for (int face = 0; face < GeometryInfo<dim>::faces_per_cell; face++) {
                if (cell->face(face)->at_boundary()) {
                    ...
                    if (cell->face(face)->boundary_id() == 2)  // on the circle
                    {
                        ...
                        for (int q = 0; q < n_q_points; q++) {
                            ...
                            fluid_stress = visc * velocity_gradients[q] - fluid_pressure;
        ...
        double aux = 2.0 / (std::pow((2.0 / 3.0) * u_max, 2) * diam_circulo);

        // Salva resultados para o passo de tempo
        coef_drag_map.insert(std::make_pair(time, aux * drag));
        coef_lift_map.insert(std::make_pair(time, aux * lift));
        dif_pressao_map.insert(std::make_pair(time, p_diff));
    }
    ...
    void grava_estatisticas() {
        ...
        std::ofstream arq_drag(dir_stats + "/drag.csv");
        arq_drag << "time,drag" << std::endl;
        for (const auto &[time, drag] : coef_drag_map) {
            arq_drag << time << "," << drag << std::endl;
        }
        arq_drag.close();

        std::ofstream arq_lift(dir_stats + "/lift.csv");
        arq_lift << "time,lift" << std::endl;
        for (const auto &[time, lift] : coef_lift_map) {
            arq_lift << time << "," << lift << std::endl;
        }
        arq_lift.close();

        std::ofstream arq_dif_pressao(dir_stats + "/dif_pressao.csv");
        arq_dif_pressao << "time,dif_pressao" << std::endl;
        for (const auto &[time, dif_pressao] : dif_pressao_map) {
            arq_dif_pressao << time << "," << dif_pressao << std::endl;
        }
        arq_dif_pressao.close();
    }
};
}  // namespace CGNS
